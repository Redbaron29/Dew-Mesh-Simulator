    ...
    if len(packets) > 0:
      ...
      for p in packets:
        ...
        for ri, rx in enumerate(rxs):
          ...
          found = False
          for pi, rxPair in enumerate(pairs.get(tx)):
          ...
          radioTypeField = "Radio Type: "+str(p.radioType)
          fields.append(radioTypeField)
...
class interactiveSim():
  ...
  def forwardPacket(self, iface, packet, rssi, snr, radioType):
  ...
  def onReceive(self, interface, packet):
    ...
    if int(packet["to"])-HW_ID_OFFSET == 4294967279:
    	dest = "all nodes"
    else:
    	dest = (packet["to"])-HW_ID_OFFSET	
    ...
    receiver = next((n for n in receivers if n.hwId != interface.myInfo.my_node_num), None)
    rxs, rssis, snrs = self.calcReceivers(transmitter, receivers)
    rP.setTxRxs(transmitter, rxs)
    rP.setRSSISNR(rssis, snrs)
    rP.setRadioType(radioType)
    if "hopLimit" in packet:
    	hopLimit = packet["hopLimit"]
    else:
    	hopLimit = None
    if packet["from"] == transmitter.hwId:
    	if hopLimit == 3:
    		if "requestId" in packet["decoded"]:
    			msgId = "Response"
    		else:
    		...
    		if "requestId" in packet["decoded"]:
    			msgId = "Retransmitted Response"
    		else:
    			msgId = "Retransmitted Original Message"
    elif packet["priority"] != "ACK":
    	if receiver.hwId == packet["from"]:
    		msgId = "an Implicit ACK"
    	else:
    		if dest == "all nodes":
    			msgId = "a Rebroadcast"
    		else:
    			msgId = "a Forwarding Packet"
    elif packet["priority"] == "ACK":
    	msgId = "an ACK"
    else:
    	msgId = "an Other"
    ...
    if packet["decoded"]["simulator"]["portnum"] == "TEXT_MESSAGE_APP":
    	if dest == "all nodes":
    		if msgId == "Original Message":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent an Original Message(text) to all other nodes. Message ID:",mId,". Radio:",radioType,"\nText: '",packet["decoded"]["simulator"]["raw"].data.decode("utf-8"),"'")
    		else:
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent",msgId,"originating from",packet["from"]-HW_ID_OFFSET,"to all other nodes. Message ID:",mId,". Radio:",radioType)
    	else:
    		if msgId == "Original Message":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent an Original Message(text) to node",dest,". Message ID:",mId,". Radio:",radioType,"\nText: '",packet["decoded"]["simulator"]["raw"].data.decode("utf-8"),"'")
    		elif msgId == "Response":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent a Response to node",dest,". Message ID:",mId,". Radio:",radioType)
    		elif msgId == "Retransmitted Response":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"Retransmitted a Response to node",dest,". Message ID:",mId,". Radio:",radioType)
    		else:
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent",msgId,"originating from node",packet["from"]-HW_ID_OFFSET,"to node",dest,". Message ID:",mId,". Radio:",radioType)
    		
    else: 
    	if dest == "all nodes":
    		if msgId == "Original Message":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent an Original Message to all other nodes. Message ID:",mId,". Radio:",radioType)
    		else:
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent",msgId,"originating from",packet["from"]-HW_ID_OFFSET,"to all other nodes. Message ID:",mId,". Radio:",radioType)
    	else:
    		if msgId == "Response":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent a Response to node",dest,". Message ID:",mId,". Radio:",radioType)
    		elif msgId == "Original Message":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent an Original Message to node",dest,". Message ID:",mId,". Radio:",radioType)
    		elif msgId == "Retransmitted Response":
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"Retransmitted a Response to node",dest,". Message ID:",mId,". Radio:",radioType)
    		else:
    			print("Node",transmitter.hwId-HW_ID_OFFSET,"sent",msgId,"originating from node",packet["from"]-HW_ID_OFFSET,"to node",dest,". Message ID:",mId,". Radio:",radioType)
    for i,r in enumerate(rxs):
    	self.forwardPacket(r.iface, packet, rssis[i], snrs[i], radioType)
